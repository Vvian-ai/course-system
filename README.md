...
「先查询是否已选，再插入」在并发下会失效——两个请求可以同时通过检查。现在在选课记录表上建立了 `(student_id, course_id)` 唯一索引作为兜底，并在服务层加上事务边界，冲突时抛出可读的业务提示。

需要注意的是：**唯一索引只能防止同一学生重复选课，防止不了超额选课**。容量判断依然是「先查人数再插入」的时序，并发下两个不同学生仍可能同时通过。要彻底解决需要乐观锁（`@Version`）或把判断写进条件 UPDATE。
